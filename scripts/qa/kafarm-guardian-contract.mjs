...
const checks = [
  ...
  { name: "Truth Reference requires a current deployment evidence boundary", ok: /deploymentBoundary/.test(truthReference) && /current-deployment-v3/.test(truthReference) && /source === "guardian_monitor" \? "UNPROVEN"/.test(truthReference) },
  ...
  { name: "Pre-rollout demo history is separated from current operational leads", ok: /demoHistoryIgnored/.test(monitor) && /demoBaselineAt/.test(monitor) && /current-deployment-v3/.test(monitor) && /current-deployment-v3/.test(truthReference) },
