...
const checks = [
  ...
  { name: "Persistent rate-limit backend is implemented", ok: /businessRpcEnforcement:\s*true/.test(rateLimitSource) && /persistentBackendInstalled:\s*true/.test(rateLimitSource) && fs.existsSync(path.join(root, "database", "applied", "078_persistent_business_rate_limit.sql")) },
  { name: "Rate-limit status fails closed until live database verification", ok: /FARMCONNECT_RATE_LIMIT_PRODUCTION_VERIFIED/.test(rateLimitSource) && /const effectiveMode = databaseVerified \? "enforce" : "unverified"/.test(rateLimitSource) },
