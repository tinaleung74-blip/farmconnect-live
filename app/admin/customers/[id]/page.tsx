...
export default function AdminCustomerDetailPage() {
  ...
  const risk = useMemo(() => {
    ...
    const rejectedCashins = cashins.filter((r) => status(r.status) === "REJECTED").length;
    const rejectedCashouts = cashouts.filter((r) => status(r.status) === "REJECTED").length;
    const rejectedSales = sellRequests.filter((r) => status(r.status) === "REJECTED").length;

    if (rejectedCashins >= 2) {
      score += 20;
      reasons.push("Multiple rejected cash-in requests");
    }

    if (rejectedCashouts >= 2) {
      score += 20;
    ...
    if (moneyNumber(customer?.wallet_balance) >= 100000) {
      score += 10;
    ...
    const level = score >= 60 ? "HIGH RISK" : score >= 25 ? "MEDIUM RISK" : "LOW RISK";
  ...
  }, [customer, cashins, cashouts, sellRequests]);
  ...
  async function suspendAccount() {
    const ok = confirm("Suspend this customer account?");
    if (!ok) return;
    ...
    <main style={page}>
      <div style={container}>
        ...
        <section style={statsGrid}>
          <Stat label="Wallet Balance" value={peso(customer.wallet_balance)} tone="green" />
        ...
        <section style={grid}>
          <section style={card}>
            ...
            <Info label="Wallet Balance" value={peso(customer.wallet_balance)} />
          </section>
          ...
          <section style={card}>
            ...
            <div style={imageGrid}>
              <KycImage
                ...
                onOpen={() => openImage(customer.id_front_url)}
              />
              <KycImage
                title="Selfie Photo"
                url={customer.selfie_url}
                onOpen={() => openImage(customer.selfie_url)}
              />
            </div>
          </section>
        ...
        <section style={actionCard}>
          ...
          <div style={actionGrid}>
            <div style={approvalBox}>
              <h3>Approve KYC</h3>
              <p>
                Mark identity as approved. Account becomes active only when
                membership is also active.
              </p>
            ...
            <div style={controlBox}>
              ...
              <button style={activateButton} onClick={activateAccount} disabled={saving}>
                🟢 Activate Account
              </button>
              <button style={suspendButton} onClick={suspendAccount} disabled={saving}>
                🚫 Suspend Account
              </button>
    ...
    <div style={imageCard}>
      ...
      {url ? (
        <>
          ...
          <button style={viewImageButton} onClick={onOpen}>
            View Full Image
          </button>
        </>
      ) : (
...
function Stat({
  ...
  label: string;
  value: string;
  tone: "green" | "blue" | "red" | "orange" | "dark";
}) {
  const color =
    tone === "green"
      ? "#16a34a"
...
function riskCard(level: string): React.CSSProperties {
  if (level === "HIGH RISK") {
    return { ...card, border: "2px solid #dc2626", background: "#fff1f2" };
  }

  if (level === "MEDIUM RISK") {
    return { ...card, border: "2px solid #f59e0b", background: "#fffbeb" };
  }
...
const viewImageButton: React.CSSProperties = {
  width: "100%",
  marginTop: 10,
  padding: 10,
  borderRadius: 12,
...
const riskScore: React.CSSProperties = {
  fontWeight: 900,
  color: "#475569",
};
