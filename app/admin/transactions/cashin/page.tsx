...
export default function AdminCashInPage() {
  ...
  async function approveCashIn(req: CashInRequest) {
    const reqStatus = status(req.status);

    if (reqStatus === "APPROVED") {
      alert("This cash-in is already approved.");
      return;
    }
    ...
    try {
      ...
      const { error: requestError } = await supabase
        ...
        .update({
          status: "APPROVED",
          admin_notes: `Approved by admin. Wallet credited ${money(amount)}.`,
      ...
      alert("Cash-in approved. Customer wallet credited.");
      await loadCashins();
    } catch (err: any) {
